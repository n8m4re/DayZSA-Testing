private ["_uid","_agent","_vars","_char","_qb","_state","_items","_itemInHands"];
...
// null = callFunction ["DataBaseWrite",format["%1_CHAR",_uid],format["%1",_char]];
// if (DB_DEBUG) then {diag_log format ["Save CHAR: %2  | UID: %1",_uid,_char]};
...
//Quickbar
{
	...
	if ( _quickIdx > -1 ) then { _qb set [_quickIdx,(typeOf _x)] };
...
// null = callFunction ["DataBaseWrite",format["%1_STATE",_uid],format["%1",_state]];
// if (DB_DEBUG) then {diag_log format ["Save STATE: %2  | UID: %1",_uid,_state]};
...
// Inventory Items [ SLOT 0 | TYPE 1 | STATE 2 [dmg 0| qty/ammo 1 | wet 2] | ITEMS 3]
{
	...
	{
		(_items select 0) set [ (count (_items select 0)),  [(typeOf _slotItem),(_slotItem call fnc_getItemState),([_agent,_slotItem] call fnc_getInvItems)]];
	};
...
{
	_items set [1,[(typeOf _itemInHands),(_itemInHands call fnc_getItemState),([_agent,_itemInHands] call fnc_getInvItems)]];
};
